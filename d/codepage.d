...
we don't do the remap. The OnTheFlyRemapNeeded() and OnTheFlyRemap(ubyte val)
...
const uint rbgDontRemapLow32=1, rbgOnlySelected=2;

alias void function(ushort[] map) TVCodePageCallBack;

// This is what usually call: TNoCaseNoOwnerStringCollection, but isn't
// available in TV, yet
class TVCodePageCol : TSortedCollection!(int*) {
	...
	//static bool compare(in CodePage *s1, in CodePage *s2) { return cmp(s1.name, s2.name) < 0; };
	static bool compare(in int *s1, in int *s2) { return cmp((cast(CodePage*)s1).name, (cast(CodePage*)s2).name) < 0; };
	override void freeItem(int *) {};
	
	override void writeItem( in int* obj, OutputStream os ) {
		//os.write(cast(ubyte[])obj);
	}
	
	override const int* readItem( InputStream inputStream ) {
		//ubyte[] str;
...
{
	char[28] name;       // Descriptive name
 	int id;              // Unique ID to identify it
	ushort[128] Font;    // High 128 symbols
 	string UpLow;       // Lowecase/Uppercase pairs
 	string MoreLetters; // Other symbols that should be treat as letters but doesn't
...
class TVCodePage {
	...
	{ return cast(char)toUpperTable[cast(ubyte)val]; }
	static char    toLower(char val)
	{ return cast(char)toLowerTable[cast(ubyte)val]; }
	static int     isAlpha(char val)
	{ return AlphaTable[cast(ubyte)val] & alphaChar; }
	static int     isNumber(char val)
	{ return AlphaTable[cast(ubyte)val] & digitChar; }
	static int     isAlNum(char val)
	{ return AlphaTable[cast(ubyte)val] & (alphaChar | digitChar); }
	static int     isLower(char val)
	{ return AlphaTable[cast(ubyte)val] & lowerChar; }
	static int     isUpper(char val)
	{ return AlphaTable[cast(ubyte)val] & upperChar; }
 ...
 // CodePage structures
	...
	static bool OnTheFlyRemapNeeded() { return NeedsOnTheFlyRemap ? true : false; }
	static ubyte   OnTheFlyRemap(int val) { return OnTheFlyMap[cast(ubyte)val]; }
	static bool OnTheFlyRemapInpNeeded() { return NeedsOnTheFlyInpRemap ? true : false; }
	static ubyte   OnTheFlyInpRemap(ubyte val) { return OnTheFlyInpMap[val]; }
	static void    CreateCPFromUnicode(CodePage *cp, int id, const char *name,
	                                   ushort *unicodes);
	static void    GetCodePages(out int idScr, out int idApp, out int idInp)
...
// are for PC437.
private static ubyte AlphaTable[256]=
[
...
private static ubyte toUpperTable[256]=
[
...
private static ubyte toLowerTable[256]=
[
 0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F,
 0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1A,0x1B,0x1C,0x1D,0x1E,0x1F,
 0x20,0x21,0x22,0x23,0x24,0x25,0x26,0x27,0x28,0x29,0x2A,0x2B,0x2C,0x2D,0x2E,0x2F,
...
// in a better place.
	static ubyte          OnTheFlyMap[256];
	static char           NeedsOnTheFlyRemap=0;
// This is a translation for the codes from keyboard.
	static ubyte          OnTheFlyInpMap[256];
	static char           NeedsOnTheFlyInpRemap=0;
// Current code pages
//  The source code is encoded in CP 437
	static int curAppCP = 437;
//  We assume the screen is also CP 437, if it isn't true the driver will inform it.
	static int            curScrCP=437;
// What we get from the keyboard
	static int            curInpCP=437;
// Default values suggested by the current driver
static int            defAppCP=437;
	static int            defScrCP=437;
	static int            defInpCP=437;
// Helpers to convert internal buffers
	static ushort         appToUnicode[256];
	static TVPartitionTree556 unicodeToApp = null;
	static ushort         inpToUnicode[256];
	static TVPartitionTree556 unicodeToInp = null;
// User provided function to call each time we change the code page.
// This is called before sending a broadcast.
	static void  function(ushort[] map) UserHook=null;
// Table used to find something that can represent an absent code.
// This is for CP 437 use
	static ubyte          Similar[]=
[
  ' ', '@', '@', '*', '*', '*', '*', '*', //   0-  7
...
[
 ...
 0x119,0x1C2,
	...
	const uint maxSymbolDefined=632+1;

	// Helper to sort Unicode tables or search in sorted Unicode tables
	static bool compare( in ref stIntCodePairs p1, in ref stIntCodePairs p2) {
 		return (p1.unicode > p2.unicode) - (p1.unicode < p2.unicode) == 0;
	}
...
static immutable CodePage stPC437 =
	{ "PC 437 ASCII ext.",
  437,
...
immutable ushort[] LowCrazyCharsRemaped =
...
immutable ushort Low32CharsRemaped[]=
...
***************************************************************************/
	...
	static void CreateCodePagesCol() {
		CodePages = new TVCodePageCol(43, 3);
	 	void a(in ref CodePage v) {
			CodePages.insert(cast(int*)&v);
	 	}
...
***************************************************************************/
	...
	this(int idApp, int idScr, int idInp) {
 		if (!CodePages) {
    		CreateCodePagesCol();
		}
 		FillTables(idApp);
...
void CreateRemap(int idSource, int idDest, ubyte *table)
{
 // Table to convert a value into an internal code.
 // That's what we need.
	ushort[] toCode=GetTranslate(idSource);
	ushort[] aux;
 ...
 ubyte[] fromCode=new ubyte[maxSymbolDefined];
 //memset(fromCode,0,maxSymbolDefined*sizeof(ubyte));
 ...
 for (uint i=0; i<128; i++)
     if (aux[i]<maxSymbolDefined) // extra check, just in case I forgot to update the constant
        fromCode[aux[i]] = cast(ubyte)(i+128);

 ubyte i=0;
 if (destCP.LowRemapNum) {
    uint to = destCP.LowRemapNum;
    aux=destCP.LowRemap;
    for (; i<to; i++)
...
void CreateOnTheFlyRemap(int idApp, int idScr)
{
 ...
 CreateRemap(idApp, idScr, OnTheFlyMap.ptr);
...
void CreateOnTheFlyInpRemap(int idInp, int idApp)
{
 // Unicode tables
 ...
 CreateRemap(idInp,idApp,OnTheFlyInpMap.ptr);
}
...
void SetCodePage(int idApp, int idScr, int idInp)
{
...
***************************************************************************/
	...
	void FillTables(int id) {
		CodePage *p = CodePageOfID(id);
 		// Initialize toUpperTable, toLowerTable and AlphaTable:
 		// 1) ASCII and defaults
  		AlphaTable[] = 0; //memset(AlphaTable,0,sizeof(AlphaTable));
		for (int i=0; i < 256; i++) {
     		if (i >= 'a' && i <= 'z') {
        		toUpperTable[i] = cast(ubyte)(i-32);
        		AlphaTable[i] = alphaChar | lowerChar;
       		} else {
        		toUpperTable[i] = cast(ubyte)i;
			}
     		if (i >= 'A' && i <= 'Z') {
				toLowerTable[i] = cast(ubyte)(i+32);
        		AlphaTable[i] = alphaChar | upperChar;
       		} else {
				toLowerTable[i] = cast(ubyte)i;
			}
     		if (i >= '0' && i <= '9') {
 ...
 // 2) For this code page
 		string s = cast(string)p.UpLow;
 		if (s !is null) {
			for (int i = 0; i < s.length; i += 2) {
				toLowerTable[s[i+1]] = s[i];
				toUpperTable[s[i]] = s[i+1];
				AlphaTable[s[i]] = alphaChar | lowerChar;
				AlphaTable[s[i+1]] = alphaChar | upperChar;
			}
		}
 		s = cast(string)p.MoreLetters;
 		if (s !is null) {
			foreach(ch; s) {
				AlphaTable[ch]=alphaChar;
			}
		}
...
***************************************************************************/
	static ccIndex IDToIndex(int id) {
 		...
 		ccIndex i437 = 0;
 		for (int i=0; i<c; i++) {
			const CodePage *p = cast(CodePage*)CodePages.at(i);
    		if (p.id == id) {
       			return i;
			}
    		if (p.id == PC437) {
...
***************************************************************************/
 ...
 if (!CodePages) return 0;
 CodePage *p=cast (CodePage *)(CodePages.at(index));
 return p.id;
}
...
***************************************************************************/
	...
	static ushort[] GetTranslate(int id) {
		if (!CodePages)
	    	CreateCodePagesCol();
 		CodePage *p = CodePageOfID(id);
 		CPTable[128..256] = p.Font[]; // memcpy(CPTable+128,p.Font,sizeof(short)*128);
		CPTable[256] = 128;
 		ubyte i = 0;
 		if (p.LowRemapNum) {
    		CPTable[256]=0;
    		CPTable[0..short.sizeof*p.LowRemapNum] = p.LowRemap[]; // memcpy(CPTable,p.LowRemap,sizeof(short)*p.LowRemapNum);
    		i=cast(ubyte)p.LowRemapNum;
   		}
 		for (; i<128; i++) {
     		CPTable[i]=i;
		}
...
***************************************************************************/
  ...
  static void GetUnicodesForCP(int id, stIntCodePairs[] unicodes) {
    ...
    sort!"(a.unicode < b.unicode)"(unicodes);
  }
...
***************************************************************************/
  ...
  static ubyte RemapChar(ubyte c, ushort[] map)
  {
   if (c < map[256])
      return c;
   int i;
...
void RemapString(ubyte *n, ubyte *o, ushort[] map) {
 int i;
 for (i=0; o[i]; i++)
...
void RemapNString(ubyte *n, ubyte *o, ushort[] map, int len) {
 for (; len; --len)
...
void RemapBufferGeneric(int sourID, int destID, ubyte *buffer, uint len,
                                    uint ops) {
...
***************************************************************************/
	...
	void FillGenericRemap(int sourID, int destID, ubyte *buffer, uint ops) {
 		if (!CodePages) {
		}
    	CreateCodePagesCol();
 		for (int i=0; i<256; i++) {
...
void RemapTVStrings(ushort[] map) {
  {
    template C(string CLA, string NAME) {
		const char[] C = "RemapString(cast(ubyte *)"~CLA~"."~NAME~", cast(ubyte *)"~CLA~".o"~NAME~", map);";
    }
   //#define C(cla,name) RemapString((ubyte *)cla::name,(ubyte *)cla::o##name,map)
   //void ();
    //C(TRadioButtons,button);
	//mixin(C!("TMenuBox", "frameChars"));

	/*mixin(C!("TFrame", "frameChars"));
	mixin(C!("TFrame", "closeIcon"));
	mixin(C!("TFrame", "zoomIcon"));
	mixin(C!("TFrame", "unZoomIcon"));
	mixin(C!("TFrame", "dragIcon"));
	mixin(C!("TFrame", "animIcon"));*/
   //C(THistory,icon);
   //C(TMonoSelector,button);
   //mixin(C!("TStatusLine", "hintSeparator"));
 ...
 {
   template C(string CLA, string NAME) {
		const char[] C = CLA~"."~NAME~" = RemapChar(" ~ CLA ~ ".o"~NAME~",map);";
    }
   ...
   mixin(C!("TMenuBox", "rightArrow"));
   //C(TRadioButtons,check);
   //C(TIndicator,modifiedStar);
   //C(TListViewer,columnSeparator);
   mixin(C!("TDesktop", "defaultBkgrnd"));
   mixin(C!("TView", "noMoireFill"));
  }

  {
  //#define C(cla,name,len) RemapNString((ubyte *)cla::name,(ubyte *)cla::o##name,map,len)
    template C(string CLA, string NAME, int LEN) {
      const char[] C = "RemapNString(cast(ubyte *)"~CLA~"."~NAME~", cast(ubyte *)"~CLA~".o"~NAME~", map, "~to!string(LEN)~");";
    }
    mixin(C!("TView", "specialChars", 6));
 ...
 if (UserHook)  {
	...
	message(null/*TProgram.deskTop*/, evBroadcast, cm.UpdateCodePage, map);
}
...
TVCodePageCallBack SetCallBack(TVCodePageCallBack cb) {
...
***************************************************************************/
	...
	static CodePage *CodePageOfID(int id) {
		return cast(CodePage*)CodePages.at(IDToIndex(id));
	}

	static immutable stIntCodePairs InternalMap[]=
[
 { 0x0020,   32 },
 { 0x0021,   33 },
 { 0x0022,   34 },
...
immutable stIntCodePairs InternalMapBrokenLinux[]=
[
 { 0x2501,  205 }, // (*1)
 { 0x2503,  186 }, // (*1)
 { 0x250f,  201 }, // (*1)
...
const int providedUnicodes = InternalMap.length;
const int providedUnicodesBL = InternalMapBrokenLinux.length;
...
static ushort UnicodeForInternalCode(ushort value) {
 int i;
...
***************************************************************************/
	...
	Optional!(immutable ushort) InternalCodeForUnicode(ushort unicode) {
		Optional!(immutable ushort) ret;
		if (!unicode) 
			return ret.some(0);
		...
		auto founds = InternalMap.find!compare(s);
	 	if (founds.length == 0) {
			founds = InternalMapBrokenLinux.find!compare(s);
		}
		return founds.length > 0 ? Some(founds[0].code) : ret.none;
	}
...
***************************************************************************/
	...
	void CreateCPFromUnicode(CodePage *cp, int id, string name, ushort[] unicodes) {
 		// Create a code page
		cp.name = name;
		cp.id=id;
	 	int i;
	 	for (i=128; i<256; i++) {
			...
			if (v == 0xFFFF) {
				...
				auto vPtr = InternalCodeForUnicode(v);
				if (vPtr.hasValue) {
					cp.Font[i-128] = v;
				} else {
					cp.Font[i-128] = 0;
...
ccIndex AddCodePage(CodePage *cp) {
 if (!CodePages) // Drivers could need to add custom cps before init.
    CreateCodePagesCol();
 return CodePages.insert(cast(int*)cp);
}
...
int LookSimilarInRange(int code, int last) {
...
static char convertU16_2_CP(int val) {
	ushort ret = unicodeToApp.search(val);
	return ret == 0xFFFF ? 0 : cast(ubyte)ret;
}
...
static ushort convertCP_2_U16(char val) {
	return appToUnicode[cast(ubyte)val];
}
...
char convertU16_2_InpCP(ushort val) {
 	if (!unicodeToInp)
    	return convertU16_2_CP(val);
	ushort ret=unicodeToInp.search(val);
	return ret==0xFFFF ? 0 : cast(ubyte)ret;
}
...
static ushort convertInpCP_2_U16(char val) {
 return inpToUnicode[cast(ubyte)val];
}
...
static void *convertBufferU16_2_CP(void *dest, in ushort[] orig, uint count) {
 ushort *o=cast(ushort *)orig;
	ubyte *d=cast(ubyte *)dest;
 while (count--)
   {
    ushort uni=unicodeToApp.search(*(o++));
		*(d++)=uni==0xFFFF ? 0 : cast(ubyte)uni;
		*(d++)=cast(ubyte)*(o++);
   }
 return dest;
}
...
static void *convertBufferCP_2_U16(void *dest, in ushort[] orig, uint count) {
	ushort *d=cast(ushort *)dest;
	ubyte *o=cast(ubyte *)orig;
 while (count--)
   {
    *(d++)=appToUnicode[*(o++)];
		*(d++)=cast(ushort)*(o++);
...
void *convertStrU16_2_CP(void *dest, const void *orig,
                                     uint len)
{
 ushort *o=cast(ushort *)orig;
 ubyte *d=cast(ubyte *)dest;
...
***************************************************************************/
	...
	ushort[] convertStrCP_2_U16(void *dest, string orig) {
		ushort[] d = new ushort[orig.length];
		//ushort *d=cast(ushort *)dest;
		//ubyte *o=cast(ubyte *)orig;
		foreach(i, ref value; d) {
			value = appToUnicode[orig[i]]; 
		}
	 	return d;
	}
